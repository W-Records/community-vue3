...
import ModifyArticleVue from '../components/ModifyArticle.vue'
...
// 全局变量
const MyArticleA = ref({});

import { getArticleDetailService,getArticleTableService } from "../api/ArticleApi.js"
// 引入全局事件总线
...
//   进入页面时，将历史数据填入
  const articleId = route.params.articleId;
    ...
    const returnArticleDetail = await getArticleTableService(articleId)
    console.log("当前文章数据为: " + returnArticleDetail);
    console.log(returnArticleDetail);

    title.value = returnArticleDetail.data.title
    editorRef.value.setHtml(returnArticleDetail.data.content)

    MyArticleA.value = returnArticleDetail.data;
    console.log(MyArticleA.value);
    ...
    <div class="headContainer">
      <div class="headButtonBars">
        ...
        <div>
          ...
          <ModifyArticleVue :articleId="route.params.articleId" :summary="MyArticleA.summary" :coverUrl="MyArticleA.coverUrl"  :categoryId="MyArticleA.categoryId" :articleTitle="title" :articleContent="content"></ModifyArticleVue>
        </div>
      </div>
    </div>
